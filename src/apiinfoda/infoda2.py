...
class Infoda():
	...
	def getInfo(self):
		q = self.s.get('http://infoda.udec.cl/INFODA_contenido.php')
		html = BeautifulSoup(q.content)
		...
		r = self.s.get('http://infoda.udec.cl/INFODA_Curricular.php')
		html2 = BeautifulSoup(r.content, 'html5lib')
		...
		d = self.s.get('http://infoda.udec.cl/INFODA_infoCurricular.php?v='+c.content)

		datos_carrera = d.content.split(';')[18].split('<td>')[1].replace("</td>'", '').split()
		codigo_carrera = datos_carrera[0]
		carrera = datos_carrera[1:]
		carrera = ' '.join(carrera)

		str_carrera = d.content.split("';")
		...
		if idasig is None:
			...
			for ramo in ramos:
				if ramo['estado'] == 'Sin Nota':
					v = self.s.get('http://infoda.udec.cl/INFODA_verMaterialAlumno.php?cAsig='+ramo['codigo'])
					html = v.content[87:].replace("'</script>", '')
	...
	def getNotas(self, idasig=None):
		...
		if idasig is not None:
			for ramo in ramos:
				if ramo['codigo'] == idasig:
					v = self.s.get('http://infoda.udec.cl/INFODA_verCalificaciones2.php?kEval=2&carr=0&cAsig='+ramo['codigo'])
					html = v.content[85:]
					html = BeautifulSoup(html)
